...
import { Check, Zap, ShoppingCart, Layout, ArrowRight } from "lucide-react"
import Link from "next/link"

export default function Website() {
  const { t } = useLang()
  const w = t.website
  const featureIcons = [Zap, Layout, ShoppingCart]
    ...
    <PageLayout>
      <section className="px-8 md:px-16 py-20 max-w-6xl mx-auto">
        ...
        <div className="fade-up text-center mb-20">
          <p className="text-neutral-500 text-sm tracking-[0.3em] uppercase mb-4">{w.badge}</p>
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-6 leading-tight">
            {w.h1a} <br /><span className="text-neutral-400">{w.h1b}</span>
          </h1>
          <p className="text-neutral-400 text-lg max-w-2xl mx-auto mb-8">{w.sub}</p>
          <div className="flex flex-wrap gap-3 justify-center">
            {["Next.js", "React", "Tailwind CSS", "Stripe", "CMS", "SEO"].map((t) => (
              <span key={t} className="px-4 py-1.5 rounded-full border border-white/10 text-neutral-400 text-xs tracking-wider">{t}</span>
            ))}
          </div>
        </div>
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-3">{w.pricingTitle}</h2>
          <p className="text-neutral-500 text-sm">{w.pricingSub}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          {w.plans.map((plan) => (
            ...
            >
              ...
              >
                {w.order} <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-3xl mx-auto mb-20">
          <h2 className="text-2xl font-bold text-white mb-8 text-center">{w.faqTitle}</h2>
          <div className="flex flex-col divide-y divide-white/10 border border-white/10 rounded-2xl overflow-hidden">
            {w.faqs.map((faq) => (
              <div key={faq.q} className="px-6 py-5 bg-white/5">
                <p className="text-white font-medium mb-2 text-sm">{faq.q}</p>
        ...
        <div className="text-center border border-white/10 rounded-2xl p-12 bg-white/5">
          <h2 className="text-3xl font-bold text-white mb-4">{w.ctaTitle}</h2>
          <p className="text-neutral-400 mb-8 max-w-md mx-auto">{w.ctaSub}</p>
          <Link
            href="/kontakti"
          ...
          >
            {w.ctaBtn} <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
